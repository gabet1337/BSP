...
public class BSP {
	...
	public List<Interval1D> GetVisibleSegments(double x, double y) { 
			...
			for (Interval1D interval : Angles.ComputeInterval(s.x1, s.y1, s.x2, s.y2, x, y)) {
				...
				for (Interval1D overlap : tree.searchAll(interval)) {
							...
							&& overlap.low <= interval.low && interval.low <= overlap.high)
					{
						...
						tree.remove(overlap);
						Interval1D newInt = new Interval1D(overlap.low, interval.low, overlap.segment);
						Interval1D newInt2 = new Interval1D(interval.high, overlap.high, overlap.segment);
						...
						tree.remove(overlap);
						Interval1D newInt = new Interval1D(interval.high, overlap.high, overlap.segment);
						tree.put(newInt, null);
					} 
		...
		for (Interval1D i : tree.searchAll(new Interval1D(0,1000,null))) {
